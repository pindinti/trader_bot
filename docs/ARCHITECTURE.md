...
The history is independent of drawing creation. An authorized member can open any readable record; restoration loads its date, timeframe, selection, drawings, and form data. Replay records additionally restore the saved replay snapshot and exclude later candles.
...
The following is a direction, not implemented behavior:

```text
active replay getMarketView() prefix
  -> independent pattern observers
